...
# Just deleting the file isn't enough - Chocolatey regenerates it during upgrade
# The only reliable fix is to fully reinstall the affected package
if (Test-Path $chocoMeta) {
    Get-ChildItem -Path $chocoMeta -Recurse -Filter "*.registry.bad" -ErrorAction SilentlyContinue | ForEach-Object {
        $corruptFound = $true
        # Extract package name from path e.g. notion.7.10.0 -> notion
...
foreach ($package in $PACKAGES) {
    ...
    if ($chocoInstalled) {
        ...
        & choco upgrade $package -y --no-progress --ignore-checksums --execution-timeout=300 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] $package upgraded." -ForegroundColor Green
        } else {
